...
const NewContent = () => {
    ...
    const get_news = async () => {
        ...
        try {
            const { data } = await axios.get(`${base_api_url}/api/news`, {
                headers: {
    ...
    const deleteNews = async (id) => {
        try {
            await axios.delete(`${base_api_url}/api/news/delete/${id}`)
            toast.success("تم مسح الخبر");
            get_news();
        } catch (error) {
            toast.error(error.response.data.message);
        }
    ...
    return (
        <div>
            <div className='px-4 py-3 flex gap-x-3'>
                <input onChange={serach_news} type="text" placeholder='ابحث عن خبر' className='px-3 py-2 rounded-md outline-0 border border-gray-300 focus:border-green-500 h-10' />
            </div>
            <div className='relative overflow-x-auto p-4'>
                <table className='w-full text-sm text-left text-slate-600'>
                    <thead className='text-xs text-gray-700 uppercase bg-gray-50'>
                        <tr>
                            <th className='px-7 py-3'>رقم</th>
                            <th className='px-7 py-3'>عنوان</th>
                            <th className='px-7 py-3'>صورة</th>
                            <th className='px-7 py-3'>فئة</th>
                            <th className='px-7 py-3'>وصف</th>
                            <th className='px-7 py-3'>تاريخ</th>
                        </tr>
                    </thead>
                    <tbody>
                        {
                            news.length > 0 && news.slice((page - 1) * parPage, page * parPage).map((n, i) => <tr key={i} className='bg-white border-b' >
                                ...
                                <td className='px-6 py-4'>
                                    <Image height={40} width={40} className='w-[40px] h-[40px]' src={n.image} alt="img" />
                                </td>
                                ...
                                <td className='px-6 py-4'>
                                    <div className='flex justify-start items-center gap-x-4 text-white'>
                                        <Link href={`/news/${n.slug}`} className='p-[6px] bg-green-500 rounded hover:shadow-lg hover:shadow-green-500/50'><FaEye /></Link>
                                        <Link href={`/edit/${n._id}`} className='p-[6px] bg-yellow-500 rounded hover:shadow-lg hover:shadow-yellow-500/50'><FaEdit /></Link>
                                        <div className='p-[6px] bg-red-500 rounded hover:shadow-lg hover:shadow-red-500/50'><FaTrash className='cursor-pointer' onClick={() => deleteNews(n._id)} /></div>
                                    </div>
                                </td>
                            </tr>)
                        }
            ...
            <div className='flex items-center justify-end px-10 gap-x-3 text-slate-600'>
                <div className='flex gap-x-3 justify-center items-center'>
                    <p className='px-4 py-3 font-semibold text-sm'>صفحة جديدة</p>
                ...
                <p className='px-6 py-3 font-semibold text-sm'>
                    {(page - 1) * parPage + 1}/{news.length} - من {pages}
                </p>
