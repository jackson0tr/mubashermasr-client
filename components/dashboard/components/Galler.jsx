...
const Galler = ({ setShow, images }) => {
    ...
    return (
        <div className='w-screen h-screen fixed right-0 top-0 z-[9999]'>
            <div className='w-full h-full relative'>
                <div className='bg-gray-400 opacity-80 w-full h-full absolute top-0 right-0 z-[998]'></div>
                    ...
                    <div className='grid grid-cols-4 gap-x-2 mt-3'>
                        {
                            images.length>0 && images.map((img,i)=><div className=' cursor-pointer' onClick={()=>copy_url(img.url)} key={i}  >
                                <Image height={100} width={100} src={img.url} alt="image" className='w-full h-[100px]' />
                            </div>)
                        }
                    </div>
                </div>
