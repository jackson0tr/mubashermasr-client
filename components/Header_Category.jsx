        ...
        <div className='w-full'>
            <div className='bg-[#c80000] w-full text-white uppercase font-semibold relative'>
                <div className='px-8 flex justify-between items-center relative h-[48px]'>
                    ...
                    <div className='flex-wrap hidden lg:flex'>
                        <Link className={`px-6 font-medium py-[13px] ${path === '/' ? 'bg-[#00000026]' : ''}`} href={'/'} >الرئيسية</Link>
                        {
                            categories.length > 0 && categories?.map((c, i) => <Link key={i} className={`px-6 font-medium py-[13px] ${path === c.name ? 'bg-[#00000026]' : ''}`} href={`/news/category/${c.name}`} >{c.name}</Link>)
                        }
                    </div>
                            ...
                            <div className='p-3 bg-white'>
                                <form onSubmit={search} className='flex'>
                                    <div className='w-[calc(100%-45px)] h-[40px]'>
                                        <input value={state} required onChange={(e) => setState(e.target.value)} type="text" placeholder='أبحث...' className='h-full w-full p-2 border border-slate-300 outline-none bg-slate-100' />
                                    </div>
            ...
            </div>
            {
                cate_show && <div className='flex flex-wrap lg:hidden py-2 px-[30px]'>
                    <Link className={`px-4 font-medium py-[5px] ${path === '/' ? 'bg-[#00000026]' : ''}`} href={'/'} >الرئيسية</Link>
                    {
                        categories.length > 0 && categories?.map((c, i) => <Link key={i} className={`px-6 font-medium py-[13px] ${path === c.name ? 'bg-[#00000026]' : ''}`} href={`/news/category/${c.name}`} >{c.name}</Link>)
                    }
                    {/* {
