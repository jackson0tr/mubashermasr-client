...
const RecentNewsFooter = async () => {
    ...
    return (
        <div className="w-full flex flex-col gap-y-[14px]">
            <div className="text-xl font-bold text-white relative before:absolute before:w-[4px] before:bg-[#c80000] before:h-full before:-right-0 pr-3">
                الأخبار المؤخرة
            </div>
                    ...
                    news && news.length > 0 && news.map((r, i) => {
                        if (i < 4) {
                            return <Link key={i} href={`/news/${r.slug}`} className="flex w-full">
                                ...
                                <div className="w-[calc(100%-90px)] pr-2">
